...
function Initialize-DeploymentContext(
  ...
  switch -Wildcard ($configurationFilePath) {
    ...
    '*.bicepparam' {
      ...
      # Move and rename the json file to the new directory
      Move-Item $configurationFilePath.Replace('.bicepparam', '.json') -Destination $newFilePath -Force
  ...
  if ($azureContext.Subscription.Id -ne $context.SubscriptionId) {
    ...
    Set-AzContext -Tenant $context.TenantId -SubscriptionId $context.SubscriptionId
  }
  if ($azureContext.Subscription.Id -ne $context.SubscriptionId) {
